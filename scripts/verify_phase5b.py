...
the three new verbs against a running REAPER and asserts real project state:
  ...
  5. spatialize_stems into a 3rd-order ambisonic SCENE (front-center / side / overhead): the scene bus
     is 16ch and each source's encoder is pointed at the fixed reference angle — az 0 / az 105 / el 45 — which
...
try:
    ...
    # ---- 5. spatialize_stems -> 3rd-order ambisonic scene (the fixed reference angles, read back live) -----
    ...
    if not have_encoder:
        ...
        # front-center -> az 0 ; side -> az +105 ; overhead -> el +45  (the fixed reference numbers)
